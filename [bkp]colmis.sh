...
ADMIN_WKSP="workspace 1" # home: "workspace 11" | performance hall: ADMIN_WKSP="workspace 1"
AUDIENCE_WKSPC="workspace 11" # home: "workspace 2" | performance hall: ADMIN_WKSP="workspace 11"
...
function setup() {
#    i3-msg assign $APP_CLASS $AUDIENCE_WKSPC
#    dim_output i3-msg "for_window [class=\"$APP_CLASS\"] move to workspace $AUDIENCE_WKSPC"
        ...
        for f in $files; do
            ...
            if [ ! -f "$download_dir/$filename" ]; then
                ...
                ( #wrapped in a subshell to be full async
                    dim_output wordcloud_cli --text $download_dir/$filename --stopwords initial-assets/stop-words.txt --width 1800 --height 860 --mode RGBA --color "#ff0000" --background "#00000040" --imagefile images/cloud.png
                    ...
                    (
                        for part in "$display_split1" "$display_split2"; do
                            dim_output magick \
                                ...
                                \( -size 1800x120 -background 'rgba(0,0,0,0.7)' -fill yellow -gravity center -font Adwaita-Sans-Bold -pointsize 45 caption:"Please fill out the online form now. Latest audience prompt:" \) \
...
#                                images/current-image.png
                    ...
                    dim_output magick \
                        ...
                        \
                        `# --- Place Yellow Title Block ---` \
                        \( -size 1800x120 -background 'rgba(0,0,0,0.7)' -fill yellow -gravity center -font Adwaita-Sans-Bold -pointsize 45 caption:"Please fill out the online form now. Last audience prompt:" \) \
                        -gravity North -geometry +0+80 -composite \
                        \
                        images/current-image.png
